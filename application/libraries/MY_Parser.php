...
class MY_Parser extends CI_Parser
{
        ...
        if ( ! empty($conditionals)) {
            foreach ($conditionals as $conditional) {
                ...
                // No compouned statement found, we're making a direct boolean check
                    $output = ( ! empty($statement) && $statement != '0' && $statement != '') ? $output : '';
                }
        ...
        foreach ($matches as $match) {
            ...
            foreach ($data as $pos => $row) {
                ...
                foreach ($row as $key => $val) {
                    ...
                    $temp[$this->l_delim.$key.$this->r_delim] = ( ! empty($val) ? $val : '');
                }
    ...
    /**
     * Replacde unparsed tags with a Parser dedicated tag ()
     * @protected
    ...
    /**
     * Removes all "unparsed" tags (i.e replaces all  tags by an empty string)
